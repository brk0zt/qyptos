...
class AddAdminView(views.APIView):
    ...
    def post(self, request, group_id):
        ...
        try:
            ...
            return Response({"error": "Kullanici bulunamadi"}, status=status.HTTP_404_NOT_FOUND)

        membership, _ = GroupMembership.objects.get_or_create(user=user, group=group)
        membership.role = "admin"
        membership.save()
        return Response({"message": f"{username} artik admin"})
...
class RemoveMemberView(views.APIView):
    ...
    def post(self, request, group_id):
        ...
        if not membership or membership.role not in ["owner", "admin"]:
            return Response({"error": "Sadece admin veya owner uye cikarabilir"}, status=status.HTTP_403_FORBIDDEN)
        ...
        return Response({"message": f"{username} gruptan cikarildi"})
...
class DeleteGroupFileView(views.APIView):
    ...
    def delete(self, request, file_id):
        ...
        if not membership or membership.role not in ["owner", "admin"]:
            return Response({"error": "Dosya silmek için admin veya owner olmalisiniz"}, status=status.HTTP_403_FORBIDDEN)
...
class AddFileCommentView(views.APIView):
    ...
    def post(self, request, file_id):
        ...
        if not GroupMembership.objects.filter(user=request.user, group=file.group).exists():
            return Response({"error": "Yorum yapmak icin grup uyesi olmalisiniz"}, status=status.HTTP_403_FORBIDDEN)

        content = request.data.get("content")
        if not content:
            return Response({"error": "Yorum bos olamaz"}, status=status.HTTP_400_BAD_REQUEST)
